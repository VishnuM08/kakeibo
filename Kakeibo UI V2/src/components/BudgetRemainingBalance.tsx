...
export function BudgetRemainingBalance({
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        {isLoading ? (
          ...
          <div className="flex items-baseline gap-2">
            <span
              className={`text-[42px] font-bold tabular-nums ${getStatusColor()}`}
            >
      ...
      {/* Stats Grid */}
      <div
        className={`grid grid-cols-2 gap-3 p-3 rounded-[12px] ${
      ...
      >
        ...
        <div>
          ...
          <p
            className={`text-[17px] font-bold tabular-nums ${isDarkMode ? "text-white" : "text-black"}`}
          >
      ...
      {!isOverBudget && daysRemaining > 0 && (
        <div
          className={`mt-4 p-3 rounded-[12px] border ${
            isDarkMode
        ...
        >
          ...
          >
            💡 You can spend <strong>₹{dailyAllowance.toFixed(2)}</strong> per
            day for the next {daysRemaining} day{daysRemaining !== 1 ? "s" : ""}{" "}
            to stay on budget.
          </p>
        </div>
      )}
