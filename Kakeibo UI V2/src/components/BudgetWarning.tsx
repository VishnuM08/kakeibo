...
export function BudgetWarning({
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[15px] text-white/90 mb-2 leading-snug">
              ...
              <span className="font-bold">${dailyAverage.toFixed(2)}</span>. At
              this rate, you'll spend ${projectedSpending.toFixed(2)} this
              month.
            </p>
            <p className="text-[13px] text-white/80">
              Suggested daily limit: $
              {((monthlyBudget - currentSpending) / daysRemaining).toFixed(2)}
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div
      className={`rounded-[20px] p-5 mb-5 shadow-sm border ${
        isDarkMode ? "bg-[#1c1c1e] border-white/10" : "bg-white border-black/12"
      }`}
    >
