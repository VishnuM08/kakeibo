...
export function BillRemindersView({
  onClose,
  isDarkMode,
  onAddExpense,
}: BillRemindersViewProps) {
  ...
  const upcomingBills = bills.filter(
    (b) => !b.isPaid && getDaysUntilDue(b.dueDate) >= 0,
  );
  ...
  // Sort by due date
  const sortedUpcoming = [...upcomingBills].sort(
    (a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime(),
  );
  const sortedOverdue = [...overdueBills].sort(
    (a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime(),
  );
  const sortedPaid = [...paidBills].sort(
        ...
        >
          <div className="flex items-center justify-between p-5">
            <div>
              ...
              <p
                className={`text-[15px] mt-1 ${isDarkMode ? "text-white/50" : "text-black/65"}`}
              >
          ...
          <div className="grid grid-cols-2 gap-3 px-5 pb-4">
            <div
              className={`p-4 rounded-[14px] ${isDarkMode ? "bg-blue-500/10" : "bg-blue-50"}`}
            >
              <p
                className={`text-[13px] font-medium mb-1 ${isDarkMode ? "text-blue-400" : "text-blue-600"}`}
              >
                Upcoming
              </p>
              <p
        ...
        >
          <div className="p-5 space-y-6">
            ...
            {bills.length === 0 && (
              ...
              >
                <Calendar
                  className={`w-16 h-16 mx-auto mb-4 ${isDarkMode ? "text-white/20" : "text-black/20"}`}
                />
                <p
                  className={`text-[17px] font-medium mb-2 ${isDarkMode ? "text-white/60" : "text-black/60"}`}
                >
                  No bills added yet
                </p>
                <p
                  className={`text-[15px] ${isDarkMode ? "text-white/40" : "text-black/40"}`}
                >
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        >
          <StatusIcon
            className={`w-5 h-5 ${getStatusColor(bill)}`}
            strokeWidth={2.5}
          />
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            <p
              className={`text-[17px] font-bold tabular-nums flex-shrink-0 ${isDarkMode ? "text-white" : "text-black"}`}
            >
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            {bill.isRecurring && (
              <>
                ...
                <p
                  className={`text-[13px] ${isDarkMode ? "text-white/40" : "text-black/40"} capitalize`}
                >
                  {bill.frequency}
                </p>
              </>
    ...
    >
      ...
      >
        <form onSubmit={handleSubmit}>
          <div
            className={`p-5 border-b ${isDarkMode ? "border-white/10" : "border-black/5"}`}
          >
            <h3
          ...
          <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
            ...
            <div>
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span
                  className={`text-[17px] font-medium ${isDarkMode ? "text-white" : "text-black"}`}
                >
          ...
          <div
            className={`p-5 border-t flex gap-3 ${isDarkMode ? "border-white/10" : "border-black/5"}`}
          >
            <button
              type="button"
              onClick={onClose}
