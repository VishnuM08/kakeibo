...
export function CalendarView({ expenses, onClose, onDateClick, isDarkMode }: CalendarViewProps) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    calendarDays.push(
      ...
      >
        <span className={`text-[17px] font-semibold mb-0.5 ${
          today 
            ? 'text-white' 
      ...
      <div className="max-w-lg mx-auto px-5 py-6">
        ...
        <div className={`rounded-[20px] p-5 mb-5 shadow-sm ${isDarkMode ? 'bg-[#1c1c1e]' : 'bg-white'}`}>
          <div className="flex items-center justify-between mb-5">
            ...
            >
              <ChevronLeft className={`w-5 h-5 ${isDarkMode ? 'text-white' : 'text-black'}`} strokeWidth={2.5} />
            </button>
            ...
            >
              <ChevronRight className={`w-5 h-5 ${isDarkMode ? 'text-white' : 'text-black'}`} strokeWidth={2.5} />
            </button>
          </div>
          ...
          <div className="grid grid-cols-7 gap-2 mb-2">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
              <div key={day} className={`text-center text-[13px] font-semibold py-2 ${isDarkMode ? 'text-white/40' : 'text-black/40'}`}>
                {day}
              </div>
            ))}
          </div>
        ...
        {/* Summary */}
        <div className={`rounded-[20px] p-5 shadow-sm ${isDarkMode ? 'bg-[#1c1c1e]' : 'bg-white'}`}>
          <h3 className={`text-[17px] font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-black'}`}>
            Monthly Summary
          </h3>
          <div className="flex items-baseline gap-2">
            <span className={`text-[34px] font-bold ${isDarkMode ? 'text-white' : 'text-black'}`}>
              ₹{expenses
                .filter(exp => {
