...
import { Expense, SavingsGoal, RecurringExpense, Budget } from '../services/api';
 ...
 */
export function queueSyncOperation(operation: Omit<SyncOperation, 'id' | 'timestamp' | 'synced'>): void {
...
export async function syncWithBackend(): Promise<void> {
  ...
  try {
    ...
    console.log('[SYNC] Pushing', pendingOps.length, 'operations to backend...');
    ...
    // TODO: BACKEND INTEGRATION - Pull latest data
...
function mergeRemoteData(remoteData: any): void {
  ...
  if (remoteData.expenses) {
    localStorage.setItem('kakeibo_expenses', JSON.stringify(remoteData.expenses));
  }
  ...
  if (remoteData.budgets) {
    ...
    if (currentBudget) {
      localStorage.setItem('kakeibo_monthly_budget', currentBudget.monthlyAmount.toString());
    }
  }
  
  if (remoteData.savingsGoals) {
    localStorage.setItem('kakeibo_savings_goals', JSON.stringify(remoteData.savingsGoals));
  }
  
  if (remoteData.recurringExpenses) {
    localStorage.setItem('kakeibo_recurring_expenses', JSON.stringify(remoteData.recurringExpenses));
  }
 ...
 */
export function saveExpenseLocally(expense: Expense): void {
 ...
 */
export function updateExpenseLocally(expense: Expense): void {
  const expenses = JSON.parse(localStorage.getItem('kakeibo_expenses') || '[]');
 ...
 */
export function getExpensesLocally(): Expense[] {
...
export function getSyncStatus(): {
  ...
  return {
    ...
    lastSyncTime: lastSyncTime ? new Date(parseInt(lastSyncTime)).toLocaleString() : null,
  };
}
